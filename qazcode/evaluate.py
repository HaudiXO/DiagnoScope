...
async def run_evaluation(
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        with Progress(
            ...
            async def process_file(json_file: Path) -> None:
                try:
...
def write_jsonl(results: list[EvaluationResult], output_path: Path) -> None:
...
def write_metrics_json(submission_name: str, metrics: dict, output_path: Path) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python evaluate.py --endpoint http://localhost:8000/diagnose --dataset-dir ./data --name my_submission
  python evaluate.py -e http://api.example.com/diagnose -d ./protocols -n team_alpha -p 10
        """,
    )
    parser.add_argument(
        "-n",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Limit evaluation to first N files (e.g. 50 for a quick test)",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
