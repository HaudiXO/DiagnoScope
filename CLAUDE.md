...
REST API (Litestar) with PostgreSQL, following Clean Architecture and DDD principles. Python 3.13, managed with UV.
...
- **di/**: Dishka dependency injection providers (`DBProvider`, `AuthProvider`)
...
- `config.yaml` — local dev (API)
- `config-local.yaml` — tests

Both are gitignored. Copy from `config-example.yaml` and fill in credentials.
