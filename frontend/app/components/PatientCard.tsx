...
export default function PatientCard({ patient, lastRun }: Props) {
    ...
    const primaryActionCls = `${actionCls} bg-[var(--color-primary)] text-[#070A06] group-hover:bg-[var(--color-primary-hover)] group-hover:shadow-[0_0_12px_rgba(161,248,0,0.25)]`;

    return (
        <Link href={`/patient/${patient.id}`} className="block group outline-none h-full">
            <Card className="flex flex-col h-full gap-3 hover:shadow-[var(--shadow-md)] transition-all group-hover:border-[var(--color-primary-soft)] group-hover:bg-[var(--color-elevated)] group-focus-visible:ring-2 group-focus-visible:ring-[var(--color-primary)]">
                ...
                <div className="flex gap-2 flex-wrap mt-auto pt-1">
                    <span className={primaryActionCls}>
                        Чат
                    </span>
                </div>
            </Card>
